...
--
-- This function creates an augroup named 'LTeXUtils' and sets up two autocommands:
-- 1. 'BufUnload': Triggers the `on_exit()` function whenever a `.tex` file is unloaded.
-- This saves the LTeX settings to a file.
-- 2. 'FileType': When a buffer of type 'tex' or 'plaintex' is opened, it sets up two user commands:
...
local function autocmd_ltex()
	...
	vim.api.nvim_command(
		"command! WriteLTeXToFile " ..
		"lua require('ltex-utils.actions').write_ltex_to_file()"
	)
	vim.api.nvim_command(
		"command! LoadLTeXFromFile " ..
		"lua require('ltex-utils.actions').load_ltex_from_file()"
	)
...
-- 
-- @return If there is an error loading the settings file and the error is not due to
--         the file not existing, it returns the error message.
--
-- @usage This function is typically called automatically when an LTeX LSP server is attached.
function M.on_attach()
	...
	cmds["_ltex.addToDictionary"] = ltex.new_handler("words", "dictionary")
...
-- 
-- @param opts A table containing user-defined options. Currently, only 'dict_path' is supported.
-- @usage
--     local ltex = require('ltex')
--     ltex.setup({dict_path = "/custom/dictionary/path"})
function M.setup(opts)
	-- use custom options if provided
	if opts and opts.dict_path then
